    ...
    this.port.receive(
      void _(var message, SendPort replyTo) {
        if (message.isStart) {
          ...
          replyTo.send(new HashServerStatus.starting(), null);
          ...
          try {
            ...
            replyTo.send(new HashServerStatus.started(_server.port), null);
            //_loggingTimer = new Timer.repeating(_handleLogging, 1000);
          } catch (var e) {
            replyTo.send(new HashServerStatus.error(e.toString()), null);
          }
        } else if (message.isStop) {
          replyTo.send(new HashServerStatus.stopping(), null);
          stop();
          replyTo.send(new HashServerStatus.stopped(), null);
        }
      });
  }
